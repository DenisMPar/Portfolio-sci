...
const fragmentShader = /* glsl */ `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    if (uGrain > 0.01) {
      ...
      vec3 grainEffect = vec3(noise * 0.4, noise, noise * 0.5) * 0.075 * uGrain * uIntensity;
    ...
    if (uScanlines > 0.01) {
      ...
      float intensity = 0.1 * uScanlines * uIntensity;
      color.rgb *= (1.0 - pattern * intensity);
      float hLines = sin(uv.y * freq * 0.1) * 0.02 * uScanlines * uIntensity;
    ...
    if (uVignette > 0.01) {
      vec2 vig = (uv - 0.5) * 2.0;
      float v = 1.0 - dot(vig, vig) * 0.3 * uVignette * uIntensity;
      color.rgb *= v;
    }
...
export const AnalogDecay = forwardRef<AnalogDecayEffect, {
  ...
  const effect = useMemo(() => new AnalogDecayEffect(props), []);
