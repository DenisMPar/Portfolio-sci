...
const chipClass =
  "inline-flex items-center gap-1.5 text-xs 2xl:text-sm border border-primary/20 px-2.5 py-1 2xl:px-3 2xl:py-1.5 text-foreground/70 font-light transition-all hover:text-foreground hover:border-primary/40 hover:shadow-[0_0_12px_rgba(80,140,200,0.15)] cursor-default";
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-baseline gap-3 sm:gap-6 py-5 px-2">
        <span className="text-xs 2xl:text-sm text-primary/70 uppercase tracking-wider sm:w-40 2xl:w-44 shrink-0 font-normal">
          {category.name}
...
function AnimatedCategoryRow({ category, isLast }: { category: (typeof skills)[number]; isLast: boolean }) {
  ...
  return (
    <m.div variants={categoryVariants}>
      <div className="flex flex-col sm:flex-row sm:items-baseline gap-3 sm:gap-6 py-5 px-2">
        <m.span className="text-xs 2xl:text-sm text-primary/70 uppercase tracking-wider sm:w-40 2xl:w-44 shrink-0 font-normal" variants={variants}>
          {category.name}
          <span className="text-accent/40 ml-2">({category.skills.length})</span>
        </m.span>
        <div className="flex flex-wrap gap-2">
